...
PHP_METHOD(amqp_class, __construct)
{
	...
	if (iniArr && SUCCESS == zend_hash_find(HASH_OF (iniArr), "login", sizeof("login"), (void*)&zdata)  && strlen(Z_STRVAL_PP(zdata)) > 0) {
		if (Z_STRLEN_PP(zdata) < 16) {
			strcpy(login, (char *)Z_STRVAL_PP(zdata));
		} else {
            zend_throw_exception(amqp_connection_exception_class_entry, "Parameter 'login' exceeds 16 characters limit.", 0 TSRMLS_CC);
            return;
        }		   
	} else { 
		strcpy(login, INI_STR("amqp.login"));
	}
	
	if (iniArr && SUCCESS == zend_hash_find(HASH_OF(iniArr), "password", sizeof("password"), (void*)&zdata)  && strlen(Z_STRVAL_PP(zdata)) > 0) {
		if (Z_STRLEN_PP(zdata) < 16) {
		    strcpy(psw, (char *)Z_STRVAL_PP(zdata));
		} else {
            zend_throw_exception(amqp_connection_exception_class_entry, "Parameter 'password' exceeds 16 characters limit.", 0 TSRMLS_CC);
            return;
        }
	} else {
		strcpy(psw, INI_STR("amqp.password"));
	}
	...
	if (iniArr && SUCCESS == zend_hash_find(HASH_OF(iniArr), "host", sizeof("host"), (void *)&zdata) && strlen(Z_STRVAL_PP(zdata)) > 0) {
		if (Z_STRLEN_PP(zdata) < 32) {
            strcpy(host, (char*)Z_STRVAL_PP(zdata));
		} else {
            zend_throw_exception(amqp_connection_exception_class_entry, "Parameter 'host' exceeds 32 characters limit.", 0 TSRMLS_CC);
            return;
		}
	} else { 
		strcpy(host, INI_STR("amqp.host"));
	}

	if (iniArr && SUCCESS == zend_hash_find(HASH_OF (iniArr), "vhost", sizeof("vhost"), (void*)&zdata)  && strlen(Z_STRVAL_PP(zdata)) > 0) {
		if (Z_STRLEN_PP(zdata) < 16) {
            strcpy(vhost, (char*)Z_STRVAL_PP(zdata));
		} else {
            zend_throw_exception(amqp_connection_exception_class_entry, "Parameter 'vhost' exceeds 16 characters limit.", 0 TSRMLS_CC);
            return;
        }
	} else {
		strcpy(vhost, INI_STR("amqp.vhost"));
	}
...
PHP_METHOD(amqp_queue_class, __construct)
{
	...
	if (!instanceof_function(Z_OBJCE_P(cnnOb), amqp_class_entry TSRMLS_CC)) {
	    zend_throw_exception(amqp_queue_exception_class_entry, "The first parameter must be and instance of AMQPConnect.", 0 TSRMLS_CC);
        return;
	} 
...
PHP_METHOD(amqp_queue_class, declare)
{
	...
	if (name_len) {
		strncpy(ctx->name,name,name_len);  
		ctx->name_len = name_len;
		amqp_name = (amqp_bytes_t) { .len = name_len, .bytes = name };
	} else {
		amqp_name = (amqp_bytes_t) { .len = ctx->name_len, .bytes = ctx->name };
	}
...
PHP_METHOD(amqp_queue_class, delete)
{
    ...
    if (name_len) {
        s = (amqp_queue_delete_t) {
            .ticket=0,
            .queue =  {.len = name_len, .bytes = name},
            .if_unused = (AMQP_IFUNUSED & parms)? 1:0,
            .if_empty = (AMQP_IFEMPTY & parms)? 1:0,
            .nowait = 0,
        };
    } else {
        s = (amqp_queue_delete_t) {
            .ticket=0,
            .queue =  {.len = ctx->name_len, .bytes = ctx->name},
            .if_unused = (AMQP_IFUNUSED & parms)? 1:0,
            .if_empty = (AMQP_IFEMPTY & parms)? 1:0,
            .nowait = 0,
        };
...
PHP_METHOD(amqp_queue_class, bind)
{
    ...
    amqp_queue_bind_t s = (amqp_queue_bind_t) {
		.ticket = 0,
		.queue = { .len = ctx->name_len, .bytes = ctx->name },
		.exchange = { .len = exchange_name_len, .bytes = exchange_name }, 
		.routing_key = { .len = keyname_len, .bytes = keyname }, 
		.nowait = 0,
...
PHP_METHOD(amqp_queue_class, consume)
{
	...
	amqp_basic_consume_t s = (amqp_basic_consume_t) {
		.ticket = 0,
		.queue = { .len = ctx->name_len, .bytes = ctx->name },
		.consumer_tag = {.len = 0, .bytes = NULL},
		.no_local = (AMQP_NOLOCAL & parms)? 1:0,
		.no_ack = (AMQP_NOACK & parms)? 1:0,
		.exclusive = (AMQP_EXCLUSIVE & parms)? 1:0,
		.nowait = 0	
	};
...
PHP_METHOD(amqp_queue_class, get)
{
	...
	amqp_basic_get_t s = (amqp_basic_get_t) {
		.ticket = 0,
		.queue = { .len = ctx->name_len, .bytes = ctx->name },
		.no_ack = (AMQP_NOACK & parms)? 1:0
	};
...
PHP_METHOD(amqp_queue_class, cancel)
{
    ...
    if (consumer_tag_len) {
		s = (amqp_basic_cancel_t) {
			.consumer_tag =  {.len = consumer_tag_len, .bytes = consumer_tag },
			.nowait = 0
		};
	} else {
		s = (amqp_basic_cancel_t) {
			.consumer_tag =  {.len = ctx->consumer_tag_len, .bytes = ctx->consumer_tag },
			.nowait = 0	
		};
	}
...
/* {{{ proto AMQPEexchange( AMQPConnect cnn, [string name]);  //////, string type=direct, [ bit params ]);
   declare Exchange   */
PHP_METHOD(amqp_exchange_class, __construct)
{
	...
	php_std_error_handling();
	if (!instanceof_function(Z_OBJCE_P(cnnOb), amqp_class_entry TSRMLS_CC)) {
	    zend_throw_exception(amqp_exchange_exception_class_entry, "The first parameter must be and instance of AMQPConnect.", 0 TSRMLS_CC);
        return;
	} 
	...
	if(!cnnOb) {
	    zend_throw_exception(amqp_exchange_exception_class_entry, "The given AMQPConnect object is null.", 0 TSRMLS_CC);
        return;
	}
...
/* {{{ proto AMQPEexchange::declare( [string name], [string type=direct], [ bit params ]);
   declare Exchange   
*/
PHP_METHOD(amqp_exchange_class, declare)
{
	...
	amqp_bytes_t amqp_type = (amqp_bytes_t) { .len = type_len, .bytes = type };
...
PHP_METHOD(amqp_exchange_class, delete)
{
	...
	if (name_len) {
        ...
        s = (amqp_exchange_delete_t) {
            .ticket = 0,
            .exchange = { .len = name_len, .bytes = name },
		    .if_unused = (AMQP_IFUNUSED & parms) ? 1:0,
		    .nowait = 0,
	    };
        ...
        s = (amqp_exchange_delete_t) {
            .ticket = 0,
            .exchange = { .len = ctx->name_len, .bytes = ctx->name },
		    .if_unused = (AMQP_IFUNUSED & parms) ? 1 : 0,
		    .nowait = 0,
		};
	}
...
PHP_METHOD(amqp_exchange_class, publish)
{
	...
	int r =	amqp_basic_publish(ctx_cnn->conn,
			    AMQP_CHANNEL,
				(amqp_bytes_t) { .len = ctx->name_len, .bytes = ctx->name },
                (amqp_bytes_t) { .len = key_len, .bytes = key_name },
                (AMQP_MANDATORY & parms) ? 1:0, // mandatory
                (AMQP_IMMEDIATE & parms) ? 1:0, // immediate
                &props,
                (amqp_bytes_t) {.len = msg_len, .bytes = msg });
...
PHP_METHOD(amqp_exchange_class, bind)
{
    ...
    amqp_queue_bind_t s = (amqp_queue_bind_t) {
		.ticket = 0,
		.queue = { .len = queue_name_len, .bytes = queue_name }, 
		.exchange = { .len = ctx->name_len, .bytes = ctx->name },
		.routing_key = { .len = keyname_len, .bytes = keyname }, 
		.nowait = 0,
		.arguments = {.num_entries = 0, .entries = NULL}
